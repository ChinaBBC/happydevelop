...
//        description.setText("需要展示的内容");
     ...
     * @param color    曲线颜色
     */
    public void showLineChart(final List<String> dataList, final List<String> dates,String name, int color) {
        ...
        xAxis.setValueFormatter(new ValueFormatter() {
            @Override
            public String getFormattedValue(float value) {
                String tradeDate = dates.get((int) value % dates.size());
                return tradeDate;
            }
        });
     ...
     * 添加曲线
     */
    public void addLine(List<String> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            String data = dataList.get(i);
            Entry entry = new Entry(i, Float.parseFloat(data));
     ...
     */
    public void resetLine(int position, List<String> dataList, String name, int color) {
